...
# 이미지 불러오기
img1 = cv2.imread("img/puppy.jpg")
img2 = cv2.imread("img/sample_puppy.jpg")

# 히스토그램 비교 후 유사도 출력
correlation = compare_histograms(img1, img2)
print(f"히스토그램 간의 상관관계: {correlation:.2f}")
# 히스토그램 간의 상관관계: 0.45   -1  ~  1 / 1(양의 상관관계 :완벽),  0(관계 없다)  , -1(음의 상관 관계 )
# 양의 상관관계가 있지만 강한상관 관계는 아니다.  -> 비교적 유사하다.
# 상관계수는 두 변수의 선형 관계의 강도와 방향 만을 측정한다.
